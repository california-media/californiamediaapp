...
import { StaffMember, fetchAllLeads, fetchCrmProjects, fetchCrmStaffs, fetchCustomers, fetchInvoices, fetchProposals, fetchTask, updateTask } from "./utils/api";
...
export default function EditTaskScreen() {
  ...
  useEffect(() => {
    ...
    Promise.all([fetchTask(taskId), fetchCrmStaffs()]).then(([t, staffs]) => {
      setStaffList(staffs);
      if (t) {
        setName(t.name || "");
        setStatus(t.status || "1");
        setPriority(t.priority || "2");
        setStartDate(parseDate(t.startdate));
        setDueDate(parseDate(t.duedate));
        setRelType(t.rel_type || "project");
        setRelId(t.rel_id || "");
        if (t.rel_type && t.rel_id) {
          setRelSelected({ id: String(t.rel_id), name: `${t.rel_type.charAt(0).toUpperCase() + t.rel_type.slice(1)} #${t.rel_id}` });
        }
        setDescription(t.description || "");
        setIsPublic(t.is_public === "1" || t.is_public === "on");
        setBillable(t.billable === "1" || t.billable === "on");
        setHourlyRate(t.hourly_rate && parseFloat(t.hourly_rate) > 0 ? t.hourly_rate : "");
        // Pre-fill assignees from task data
        if (Array.isArray(t.assignees)) {
          const assignees: StaffMember[] = t.assignees
            .filter((a: any) => a.staffid || a.assigneeid)
            .map((a: any) => ({
              staffid: Number(a.staffid || a.assigneeid),
              firstname: a.firstname || "",
              lastname: a.lastname || "",
              email: a.email || "",
            }));
          setSelectedAssignees(assignees);
        } else if (Array.isArray(t.assigned)) {
          const assignees: StaffMember[] = t.assigned.map((a: any) => ({
  ...
  // Clear related when type changes (only if user triggered)
  const handleRelTypeChange = (t: string) => {
    setRelType(t);
    setRelSelected(null);
    setRelId("");
    setRelSearch("");
      ...
      try {
        ...
        if (relType === "project") {
          ...
          results = data
            ...
            .slice(0, 8).map(p => ({ id: String(p.id), name: p.name }));
          ...
          results = data
            ...
            .slice(0, 8).map(i => ({ id: String(i.id), name: `${i.prefix || ""}${i.number || i.id}` }));
          ...
          results = data
            ...
            .slice(0, 8).map(p => ({ id: String(p.id), name: p.subject || `Proposal #${p.id}` }));
        }
  ...
  const validate = () => {
    ...
    if (startDate && !startDate.match(/^\d{4}-\d{2}-\d{2}$/)) errs.startDate = "Use YYYY-MM-DD";
  ...
  const handleSave = async () => {
    ...
    try {
      const result = await updateTask(taskId, {
        ...
        is_public: isPublic ? "on" : "0",
        billable: billable ? "on" : "0",
        ...
        >
          ...
          <Section title="Assigned To">
            ...
            <TouchableOpacity style={styles.pickerToggle} onPress={() => setShowAssigneePicker((v) => !v)}>
              ...
              <Text style={styles.pickerToggleText}>{showAssigneePicker ? "Close" : "Change Assignees"}</Text>

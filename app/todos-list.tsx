...
const fetchTodosPage = async (finished: 0 | 1): Promise<Todo[]> => {
  const res = await fetch(`${getCrmApiUrl()}/todos/listing?page=0&finished=${finished}`, {
    method: "GET",
...
const updateTodo = async (id: string, description: string, finished: string): Promise<boolean> => {
  const form = new FormData();
  form.append("description", description);
...
export default function TodosListScreen() {
  ...
  const closeModal = () => { setModalVisible(false); setEditingTodo(null); setInputText(""); };
  ...
  const handleToggle = async (todo: Todo) => {
    ...
    try {
      ...
      if (ok) {
        setTodos((prev) =>
          prev.map((t) => t.todoid === todo.todoid ? { ...t, finished: newFinished } : t)
        );
      }
    } catch { /* silent */ }
    ...
    return (
      <View style={styles.card}>
        <View style={styles.cardRow}>
          ...
          >
            {toggling ? (
              <ActivityIndicator size={12} color={isDone ? "#fff" : "#6366f1"} />
            ) : isDone ? (
          ...
          <View style={styles.cardRight}>
            <View style={[styles.statusPill, { backgroundColor: isDone ? "#d1fae5" : "#fef3c7" }]}>
              <View style={[styles.statusDot, { backgroundColor: isDone ? "#10b981" : "#f59e0b" }]} />
              <Text style={[styles.statusText, { color: isDone ? "#059669" : "#d97706" }]}>
                {isDone ? "Done" : "Pending"}
              </Text>
            </View>
            <View style={styles.actionRow}>
              <TouchableOpacity style={styles.editBtn} onPress={() => openEdit(item)} hitSlop={{ top: 6, bottom: 6, left: 6, right: 6 }}>
    ...
    <>
      ...
      <View style={styles.hero}>
        ...
        <View style={styles.heroTop}>
          ...
          <TouchableOpacity style={styles.addHeroBtn} onPress={openAdd} activeOpacity={0.85}>
        ...
        <View style={styles.heroStats}>
          {[
            { label: "All", count: counts.all, color: "#fff", bg: "rgba(255,255,255,0.18)" },
            { label: "Pending", count: counts.pending, color: "#fef3c7", bg: "rgba(245,158,11,0.25)" },
            { label: "Done", count: counts.done, color: "#d1fae5", bg: "rgba(16,185,129,0.25)" },
          ].map((s) => (
        ...
        ListEmptyComponent={
          <View style={styles.emptyBox}>
            ...
            <Text style={styles.emptyTitle}>
              {activeTab === "done" ? "No Completed Todos" : activeTab === "pending" ? "No Pending Todos" : "No Todos Yet"}
            </Text>
          ...
          <View style={styles.modalSheet}>
            ...
            >
              {saving ? (
                ...
                <>
                  <Ionicons name={editingTodo ? "checkmark-circle" : "add-circle"} size={18} color="#fff" />
                  <Text style={styles.saveBtnText}>{editingTodo ? "Save Changes" : "Add Todo"}</Text>
                </>
              )}
            </TouchableOpacity>
          </View>
...
const styles = StyleSheet.create({
  ...
  cbPending: { backgroundColor: "transparent", borderWidth: 2, borderColor: "#cbd5e1" },
  ...
  textInput: {
    ...
    fontSize: 15, color: "#0f172a", minHeight: 80, marginBottom: 20,
  },
  saveBtn: {
  ...
  errorTitle: { fontSize: 17, fontWeight: "700", color: "#1e293b", marginTop: 14 },
  errorSub: { fontSize: 14, color: "#64748b", textAlign: "center", marginTop: 6, marginBottom: 24 },
  retryBtn: { backgroundColor: "#6366f1", paddingHorizontal: 28, paddingVertical: 13, borderRadius: 12 },
